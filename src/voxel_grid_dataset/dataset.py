...
def T_to_xyzrpy(T):
    ...
    pitch = -math.asin(
        rotation_matrix[2, 0]
    )  # - sign because of different rotation conventions
    ...
    # Convert pitch, yaw, and roll to degrees if desired
...
class TwoVoxelGridsTwoTransformsDataset(Dataset):
    def __init__(
        self,
        name=None,
        mode="read",
        ...
        if self.mode == "read":
            ...
            assert "voxel_size" in identifiers.keys()
    ...
    def load_dataset(self):
        ...
        self.vg1_general = torch.zeros(
            size=(self.input_manager.n_datapoints,1, length, width, height),dtype=torch.float32
        )
    ...
    def gen_all_labels(self):
        ...
        for i in range(self.input_manager.n_datapoints):
            self.labels[i, :] = torch.Tensor(self.gen_label(self.poses1[i], self.poses2[i]))
            self.labels[i + self.input_manager.n_datapoints, :] = torch.Tensor(self.gen_label(
                self.poses2[i], self.poses1[i]
            ))
